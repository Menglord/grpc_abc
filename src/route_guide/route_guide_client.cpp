...
public:
	...
	void ListFeature() {
		...
		std::cout << "Looking for features between 40, -75 and 32, -73" << std::endl;

		std::unique_ptr<ClientReader<Feature> > reader(stub_->ListFeature(&context, rect));
	...
	void RecordRoute() {
		...
		std::uniform_int_distribution<int> delay_distrubution(500, 1000);
		...
		for (int i = 0; i < kPoints; i++) {
			...
			std::this_thread::sleep_for(std::chrono::milliseconds(delay_distrubution(generator)));
		}
		writer->WritesDone();
		...
		if (status.ok()) {
			std::cout << "Finished trip with " << stats.point_count() << " points\n"
					...
					<< "It took " << stats.elapsed_time() << " secondes" << std::endl;
		} else {
